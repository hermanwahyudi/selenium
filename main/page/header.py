...
class HeaderPage(BasePage):
    ...
    _search_button_loc = (By.CSS_SELECTOR, 'div.search-parent div.cat-wrapper span.btn-search-wrapper button.btn-search')
    ...
    #USER LOCATOR
    _user_loc = (By.CSS_SELECTOR, 'li#user-tab a.dropdown-toggle img.imgprofile')
    #--
    ...
    def check_header_elements(self, flag):
        ...
        if flag == 1:
            ...
            for each_header_bar in self._guest_header:
                ...
                print ("Header Title found : " + "'"+ self.find_element(*self._guest_header[each_header_bar]).text + "'")
                self.mouse_hover_to(*self._guest_header[each_header_bar])
    ...
    def mouse_hover_to_user_bar(self):
        return self.mouse_hover_to(*self._user_loc)
    ...
    def mouse_hover_and_go_to_inbox_resolution_center_notification(self):
        notif_inbox_resolution_center = self.mouse_hover_to(*self._notification_header['inbox_resolution_center_notification_loc'])
    ...
    def mouse_hover_and_go_to_myshop_order_status_notification(self):
        notif_myshop_order_status = self.mouse_hover_to(*self._notification_header['myshop_order_status_notification_loc'])
...
#Check header status
def check_header_status(driver, flag):
...
def check_header_of_page(driver):
    header_page = HeaderPage(driver)
