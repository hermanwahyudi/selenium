...
class HeaderPage(BasePage):
    ...
    _guest_header = {
        ...
        'login_loc': (By.XPATH, '//*[@id="login-ddl-link"]')
    }
    ...
    def do_login(self, email, pwd):
        login_at_header = self.find_element(*self._guest_header['login_loc'])
        self.check_visible_element(*self._guest_header['login_loc'])
        self.click_on_javascript(login_at_header)

        self.check_visible_element(*self._guest_login_header['email_loc'])
        input_email = self.find_element(*self._guest_login_header['email_loc']).send_keys(email)

        self.check_visible_element(*self._guest_login_header['email_loc'])
        input_pwd = self.find_element(*self._guest_login_header['pwd_loc']).send_keys(pwd)

        self.check_visible_element(*self._guest_login_header['email_loc'])
        submit = self.find_element(*self._guest_login_header['submit_loc']).click()
