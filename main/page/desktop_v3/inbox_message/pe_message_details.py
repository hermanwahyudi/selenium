...
class PageMessageDetails(InboxMessagePage):
	...
	#replies
	_reply_loc = (By.CSS_SELECTOR, 'div.span12 form div div div')
	...
	def click_reply_button(self):
		self.find_element(*self._reply_btn_loc).click()

	def click_back_button(self):
		self.find_element(*self._back_btn_loc).click()

	def write_reply(self, message):
		self.find_element(*self._textarea_loc).send_keys(message)

	def get_reply_ID(self):
		replies = self.find_elements(*self._reply_loc)
		reply_ID_list = []
		for i in replies:
			reply_ID_list.append(i.get_attribute('id'))
